classify_emotion <- function(textColumns,algorithm="bayes",prior=1.0,verbose=FALSE,...) {
  ...
  #print(dim(lexicon))
  #print((lexicon[,1]))
  counts <- list(anger=length(which(lexicon[,1]=="anger")),disgust=length(which(lexicon[,1]=="disgust")),fear=length(which(lexicon[,1]=="fear")),joy=length(which(lexicon[,1]=="joy")),sadness=length(which(lexicon[,1]=="sadness")),surprise=length(which(lexicon[,1]=="surprise")),total=nrow(lexicon))
  documents <- c()
  ...
  for (i in 1:nrow(matrix)) {
    ...
    for (word in words) {
      for (key in names(scores)) {
        emotions <- lexicon[which(lexicon[,1]==key),]
        str(emotions)
        index <- pmatch(word,emotions,nomatch=0)
        if (index > 0) {
